...
This local setup is browser-only, so prefer explicit `--engine browser` runs and do not use API mode here.
...
3. Run in browser mode for the usual GPT‑5.4 Pro ChatGPT workflow.
...
- This setup is browser-only. Pass `--engine browser` explicitly, even if `OPENAI_API_KEY` is present in the environment.
- Browser engine supports GPT + Gemini only.
- Browser attachments:
